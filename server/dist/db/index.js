...
const dbPath = path_1.default.join(dbDir, 'cadenza.db');
...
const presetCount = exports.db.prepare('SELECT COUNT(*) as count FROM presets').get();
if (presetCount.count === 0) {
    const insertPreset = exports.db.prepare(`
  ...
  `);
    ...
    for (const preset of defaultPresets) {
        insertPreset.run(preset.id, preset.name, preset.description, preset.category, preset.config_json, preset.is_default);
    }
}
